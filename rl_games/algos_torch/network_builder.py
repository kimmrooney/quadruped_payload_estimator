...
class EstimatorBuilder(A2CBuilder):
    """
    YAML 파일에서 'builder: name: estimator_builder'를 지정했을 때,
    ...
    class Network(A2CBuilder.Network):
        ...
        def forward(self, obs_dict):
            # 이 메소드는 부모 클래스의 forward 로직을 기반으로,
            # 페이로드 추정 및 손실 계산 부분을 추가한 것입니다.
            is_train = obs_dict.get('is_train', True)

            # 1. 기본 Actor-Critic 순전파 (A2CBuilder.Network 로직과 거의 동일)
            out = obs_dict['obs']
            out = self.actor_cnn(out)
            out = out.flatten(1)
            
            # 공통 몸통 MLP를 통과하여 common_feature를 만듭니다.
            ...
            if is_train and 'ground_truth_payload' in obs_dict:
